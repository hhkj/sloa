...
function InitTopMenu() {
	$.each(_menus, function(i, n) {
		if (n.parentMenu == "0") {
			...
			if (i == 0) {
				str += '<a class="item active" target="LeftList" id="tommenu_'
						+ n.menuid + '" onClick="InitLeftMenu(\'' + n.menuid
						+ '\')"  onfocus="this.blur();">';
			} else {
				str += '<a class="item" target="LeftList" id="tommenu_'
						+ n.menuid + '" onClick="InitLeftMenu(\'' + n.menuid
						+ '\')"  onfocus="this.blur();">';
			}
...
function InitLeftMenu(parentMenu) {
	...
	for (var i = 0; i < l; i++) {
	 ...
	 $.each(_menus, function(i, n) {
		 if(n.menuid==parentMenu){
			 $.each(n.menus, function(i, k) {
				 var menulist ='';
				 menulist +='<ul>';
				 $.each(k.menus, function(j, o) {
...
function getMenu(menuid) {
	var m = null;
	$.each(_menus, function(i, k) {
		$.each(k.menus, function(i, n) {
...
function addTab(menuid) {
	...
	if ($("#center_iframe")) {
		...
		$('#layout_center_plan')
				.panel(
						'setTitle',
						'<ul class="navigation-path"><li><a href="javascript:;" onclick="addFristPage()" >首页</a></li><li>'
								+ opts.title + '</li></ul>');
	} else {
		createFrame(opts);
		opts['closable'] = false;
...
function createFrame(opts) {
	var url = opts.href;
	var $iframe = $("<iframe id='center_iframe' name='" + opts.title
			+ "'  class='main_iframe' src='" + url
...
function addFristPage() {
	var $iframe = $("<iframe id='center_iframe' class='main_iframe' border='1'  scrolling='auto' src='../kanban.jsp' width='100%' height='100%'></iframe>");
	$('#layout_center_plan')
			.panel(
					{
